...
class Matrix:
    ...
    def LU_Inverse(self):
        E = scisp.lil_matrix((self.n, self.n))
        for i in range(self.n):
            E[i, i] = 1

        x = scisp.lil_matrix((self.n, self.n))
        for step in range(self.n):
            y = scisp.lil_matrix((1, self.n))
            y[0, 0] = E[0, step] / self.decomposed_lower[0, 0]
            for i in range(1, self.n):
                y[0, i] = (E[i, step] - sum([self.decomposed_lower[i, k] * y[0, k] for k in range(i)])) / \
                          self.decomposed_lower[i, i]

            x[self.n - 1, step] = y[0, self.n - 1] / self.decomposed_upper[self.n - 1, self.n - 1]
            for i in range(self.n - 2, -1, -1):
                x[i, step] = (y[0, i] - sum([self.decomposed_upper[i, k] * x[k, step] for k in range(i + 1, self.n)])) / \
